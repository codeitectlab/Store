...
BEANSTALK_APPLICATION_NAME=store-space
BEANSTALK_ENVIRONMENT_NAME[0]=
BEANSTALK_ENVIRONMENT_NAME[1]=store-space-com-01
BEANSTALK_ENVIRONMENT_NAME[2]=store-space-com-02

AWS="aws --profile suchi --region us-east-1"

S3_BUCKET=store-space-com
S3_FOLDER=website
...
CNAME_1=`$AWS --output text elasticbeanstalk describe-environments --query 'Environments[0].{CNAME: CNAME}' --application-name store-space --environment-name ${BEANSTALK_ENVIRONMENT_NAME[1]}`
CNAME_2=`$AWS --output text elasticbeanstalk describe-environments --query 'Environments[0].{CNAME: CNAME}' --application-name store-space --environment-name ${BEANSTALK_ENVIRONMENT_NAME[2]}`

PROD=1
QA=2

if [[ "$CNAME_1" =~ "store-space-com-test" ]]; then
	QA=1
fi

if [[ "$CNAME_2" =~ "store-space-com-prod" ]]; then
	PROD=2
fi

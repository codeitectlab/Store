...
let controller = {
  ...
  console.logger.debug(reservation);
  ...
  insurance: (req, res) => {
    ...
    console.logger.debug(reservation.insurance); //TWP
  ...
  online: (req, res) => {
    ...
    let params = {
      sLocationCode: reservation.site.sLocationCode,
    ...
    console.logger.debug(req.body.insuranceId);
    ...
    soap.moveInCostRetrieveWithDiscount(params).then((result) => {
      ...
      console.logger.debug(reservation.moveIn);
      console.logger.debug(reservation.moveIn.total);
  ...
  esign: (req, res) => {
    ...
    console.logger.debug(reservation); //TWP
    ...
    soap.tenantList(params).then((result) => {
      ...
      console.logger.debug(reservation.tenant);

      let decimalvalue = parseFloat(reservation.moveIn.total);
      //let today = '2018-07-04T00:00:00'; //TWP Testing
      ...
      let moveInParams = {
        ...
        dStartDate: DateTime.local().toISO(), /*reservation.moveIn.startDate,*/
        dEndDate: reservation.moveIn.endDate,
        dcPaymentAmount: parseFloat(decimalvalue) /*reservation.moveIn.total*/,
        ...
        InsuranceCoverageID: reservation.insurance.InsurCoverageID,
        ConcessionPlanID: reservation.unitType.ConcessionID,
      };

      console.log(reservation.tenant.TenantID); //added by TWP
      console.logger.debug(reservation.moveIn.total)
...
//      predicates.push('sFormName like \'%Insurance Sign-up%\'')
      ...
      console.log(reservation.tenant.TenantID);
...
//    let unitInfo = req.app.units[req.body.unitId];
    ...
    soap.tenantList(params).then((result) => {
      let tenants = result.TenantListDetailed_v2Result.diffgram.NewDataSet.Table;
      let tenant = null;

      console.logger.info(tenants);

      if(tenants) {
        for(let i=0; i<tenants.length; i++) {
          let found = true;
          Object.keys(params).forEach(key => {
            if(!tenants[i][key] || params[key].toLowerCase() !== tenants[i][key].toLowerCase())
              found = false;
          });

          if(found) return tenants[i];
        }
      }
      ...
      return soap.createTenant(tenantParams).then((r) => {
        console.logger.info(r.TenantNewDetailedResult.diffgram.NewDataSet.Tenants);
        return r.TenantNewDetailedResult.diffgram.NewDataSet.Tenants;
      });
    }).then((result) => {
      tenantInfo = result;

      console.logger.info(result);
  ...
  getReservationModel: (s, u, ut, t, i, m, c, e) => {
    return {
      ...
      moveIn: m || {
        costs: [],
        endDate: new Date(),
        startDate: new Date().setFullYear(2030),
        taxes: 0.0,
        total: 0.0,
      },
      site: s || {},
